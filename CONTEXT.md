...
**Screen (大屏)**:
可独立创建、命名和访问的监控展示资源，包含自己的画布编排、组件实例和默认监控对象指派。同一平台可管理多份大屏，每份大屏拥有独立且稳定的访问入口。
- _避免使用 (Avoid)_: 把大屏资源称为组件模板或画布副本
...
**Embedded Display (嵌入展示)**:
由其他业务系统承载指定大屏的展示方式，保留实时监控以及目标和来源切换。切换只影响当前展示页面，重新打开时恢复大屏已保存的默认指派。
- _避免使用 (Avoid)_: 嵌入编辑器, 截图展示, 静态预览
...
在大屏画布就地切换或右侧属性面板进行槽位指派时，同时选择业务目标（如弹目标批号）与其物理数据来源（如雷达、遥测等传感器站）。两处入口操作同一份实例指派，作用于该槽位下属的关联控件。组件工坊的目标和来源选择仅用于临时预览，不属于大屏实例指派。
- _避免使用 (Avoid)_: 单实体硬切, 来源与目标分离绑, 槽位暗箱过滤

**Business Target (业务目标)**：现场关注的同一个飞行器、船舶等业务对象。同一业务目标可能被多个传感器观测，目标选择列表应合并展示。

**Source Record (来源记录)**：某个数据来源对业务目标的独立观测记录，拥有自己的数值、更新时间和历史。不同来源的历史不得拼接为一条曲线。

**Workshop Preview (工坊预览)**：设计模板时临时选择业务目标和来源以查看控件效果。选择仅作用于当前控件的相关槽位，不进入模板或实例的持久化配置。
