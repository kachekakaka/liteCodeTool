...
export default defineComponent({
  ...
  setup(props) {
    ...
    function startEditText() {
      ...
      nextTick(() => {
        const el = (document.activeElement?.closest('.atomic-text') || document.querySelector(`[data-control="${props.control.id}"]`))?.querySelector('.inline-text-input') as HTMLInputElement;
        el?.focus();
        el?.select();
      });
    }
    ...
    return { editingText, textDraft, startEditText, commitText, cancelText, tableHover, statusClass, state, c, value, statusColor, columns, rows, page, pageSize, pageCount, shown, image, imageFailed, stamp, formatScalar, isRowStale };
  }
});
</script>
<template>
  <div class="atomic" :class="['atomic-'+c.type, { 'is-stale': value.stale, 'atomic-kpi': c.props.variant==='kpi' }]" @mouseenter="tableHover=true" @mouseleave="tableHover=false">
    ...
    <template v-else-if="c.type==='text'">
      <input v-if="editingText" v-model="textDraft" class="inline-title-input inline-text-input" autofocus @keydown.enter.stop="commitText" @keydown.esc.stop="cancelText" @blur="commitText" @pointerdown.stop @click.stop @dblclick.stop/>
      <span v-else class="text-value" :class="{ 'editable-text': c.props.sourceMode !== 'dynamic' && !c.props.clock }" :title="c.props.sourceMode !== 'dynamic' && !c.props.clock ? '双击就地修改文本' : value.value" @dblclick.stop="startEditText">{{ value.value }}</span>
